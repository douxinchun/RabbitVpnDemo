...
open class DNSServer: DNSResolverDelegate, IPStackProtocol {
    ...
    fileprivate func lookup(_ session: DNSSession) {
        ...
        case .real, .unknown:
            NSLog("DNSServer real unkonwn")

            lookupRemotely(session)
        default:
            DDLogError("DNSServer The rule match result should never be .Pass.")
        }
    }
    ...
    open func input(packet: Data, version: NSNumber?) -> Bool {
        ...
        guard IPPacket.peekDestinationAddress(packet) == serverAddress else {
            NSLog("DNSServer now serverAddress \(String(describing: IPPacket.peekDestinationAddress(packet)))")
            NSLog("DNSServer !=serverAddress \(serverAddress)")
            return false
        }

        guard IPPacket.peekDestinationPort(packet) == serverPort else {
            NSLog("DNSServer !=port \(serverPort)")
            return false
        }

        guard let ipPacket = IPPacket(packetData: packet) else {
            NSLog("DNSServer !=IPPacket")
            return false
        }

        guard let session = DNSSession(packet: ipPacket) else {
            NSLog("DNSServer DNSSession is nil")
            return false
        }
        NSLog("DNSServer input")
        queue.async {
            self.lookup(session)
        }
        return true
