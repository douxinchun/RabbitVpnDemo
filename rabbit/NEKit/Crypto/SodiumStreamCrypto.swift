...
open class SodiumStreamCrypto: StreamCryptoProtocol {
    ...
    open func update(_ data: inout Data) {
        ...
        switch algorithm {
        case .chacha20:
            let outputDataLocal = outputData
            _ = outputData.withUnsafeMutableBytes { outputPtr in
                iv.withUnsafeBytes { ivPtr in
                    key.withUnsafeBytes { keyPtr in
                        crypto_stream_chacha20_xor_ic(outputPtr, outputPtr, UInt64(outputDataLocal.count), ivPtr, UInt64(counter/blockSize), keyPtr)
                    }
                }
            }
        ...
        case .salsa20:
            ...
            _ = outputData.withUnsafeMutableBytes { outputPtr in
                iv.withUnsafeBytes { ivPtr in
                    key.withUnsafeBytes { keyPtr in
                        crypto_stream_salsa20_xor_ic(outputPtr, outputPtr, UInt64(outputDataLocal.count), ivPtr, UInt64(counter/blockSize), keyPtr)
                    }
                }
            }
        }
